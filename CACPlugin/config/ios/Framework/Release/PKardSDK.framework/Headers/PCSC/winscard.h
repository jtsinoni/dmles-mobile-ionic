//
//  winscard.h
//  PKardSDK
//
//  Created by Paul Nelson on 6/8/12.
//  Copyright (c) 2012 Thursby Software Systems, Inc. All rights reserved.
//

#ifndef PKardSDK_winscard_h
#define PKardSDK_winscard_h

#include <PKardSDK/PCSC/wintypes.h>

#ifdef __cplusplus
extern "C"
{
#endif

#pragma mark PCSC Error Codes
/*
 * error codes
 */
#define ERROR_BROKEN_PIPE   0x00000109  /* The client attempted a smart card operation in a
remote session, such as a client session running 
on a terminal server, and the operating
system in use does not support smart card redirection. */
#define SCARD_E_BAD_SEEK                0x80100029  /* An error occurred in setting the smart card file object pointer. */
#define SCARD_E_CANCELLED               0x80100002  /* The action was canceled by an SCardCancel request. */
#define SCARD_E_CANT_DISPOSE            0x8010000E  /* The system could not dispose of the media in the requested manner. */
#define SCARD_E_CARD_UNSUPPORTED        0x8010001C  /* The smart card does not meet minimal requirements for support. */
#define SCARD_E_CERTIFICATE_UNAVAILABLE 0x8010002D  /* The requested certificate could not be obtained. */
#define SCARD_E_COMM_DATA_LOST          0x8010002F  /* A communications error with the smart card has been detected. */
#define SCARD_E_DIR_NOT_FOUND           0x80100023  /* The specified directory does not exist in the smart card. */
#define SCARD_E_DUPLICATE_READER        0x8010001B  /* The reader driver did not produce a unique reader name. */
#define SCARD_E_FILE_NOT_FOUND          0x80100024  /* The specified file does not exist in the smart card. */
#define SCARD_E_ICC_CREATEORDER         0x80100021  /* The requested order of object creation is not supported. */
#define SCARD_E_ICC_INSTALLATION        0x80100020  /* No primary provider can be found for the smart card. */
#define SCARD_E_INSUFFICIENT_BUFFER     0x80100008  /* The data buffer for returned data is too small for the returned data. */
#define SCARD_E_INVALID_ATR             0x80100015  /* An ATR string obtained from the registry is not a valid ATR string. */
#define SCARD_E_INVALID_CHV             0x8010002A  /* The supplied PIN is incorrect. */
#define SCARD_E_INVALID_HANDLE          0x80100003  /* The supplied handle was not valid. */
#define SCARD_E_INVALID_PARAMETER       0x80100004  /* One or more of the supplied parameters could not be 
properly interpreted. */
#define SCARD_E_INVALID_TARGET          0x80100005  /* Registry startup information is missing or not valid. */
#define SCARD_E_INVALID_VALUE           0x80100011  /* One or more of the supplied parameter values could
not be properly interpreted. */
#define SCARD_E_NO_ACCESS               0x80100027  /* Access is denied to the file. */
#define SCARD_E_NO_DIR                  0x80100025  /* The supplied path does not represent a smart card directory. */
#define SCARD_E_NO_FILE                 0x80100026  /* The supplied path does not represent a smart card file. */
#define SCARD_E_NO_KEY_CONTAINER        0x80100030  /* The requested key container does not exist on the smart card. */
#define SCARD_E_NO_MEMORY               0x80100006  /* Not enough memory available to complete this command. */
#define SCARD_E_NO_PIN_CACHE            0x80100033  /* The smart card PIN cannot be cached.  
Windows Server 2008, Windows Vista, Windows Server 2003, and Windows XP:
This error code is not available. */
#define SCARD_E_NO_READERS_AVAILABLE    0x8010002E  /* No smart card reader is available. */
#define SCARD_E_NO_SERVICE              0x8010001D  /* The smart card resource manager is not running. */
#define SCARD_E_NO_SMARTCARD            0x8010000C  /* The operation requires a smart card, but no smart card
is currently in the device. */
#define SCARD_E_NO_SUCH_CERTIFICATE     0x8010002C  /* The requested certificate does not exist. */
#define SCARD_E_NOT_READY               0x80100010  /* The reader or card is not ready to accept commands. */
#define SCARD_E_NOT_TRANSACTED          0x80100016  /* An attempt was made to end a nonexistent transaction. */
#define SCARD_E_PCI_TOO_SMALL           0x80100019  /* The PCI receive buffer was too small. */
#define SCARD_E_PIN_CACHE_EXPIRED       0x80100032  /* The smart card PIN cache has expired.
Windows Server 2008, Windows Vista, Windows Server 2003, and Windows XP:
This error code is not available. */
#define SCARD_E_PROTO_MISMATCH          0x8010000F  /* The requested protocols are incompatible with the 
protocol currently in use with the card. */
#define SCARD_E_READ_ONLY_CARD          0x80100034  /* The smart card is read-only and cannot be written to.
Windows Server 2008, Windows Vista, Windows Server 2003, and Windows XP:
This error code is not available. */
#define SCARD_E_READER_UNAVAILABLE      0x80100017  /* The specified reader is not currently available for use. */
#define SCARD_E_READER_UNSUPPORTED      0x8010001A  /* The reader driver does not meet minimal requirements for support. */
#define SCARD_E_SERVER_TOO_BUSY         0x80100031  /* The smart card resource manager is too busy to complete this operation. */
#define SCARD_E_SERVICE_STOPPED         0x8010001E  /* The smart card resource manager has shut down. */
#define SCARD_E_SHARING_VIOLATION       0x8010000B  /* The smart card cannot be accessed because of other outstanding connections. */
#define SCARD_E_SYSTEM_CANCELLED        0x80100012  /* The action was canceled by the system, presumably to log off or shut down. */
#define SCARD_E_TIMEOUT                 0x8010000A  /* The user-specified time-out value has expired. */
#define SCARD_E_UNEXPECTED              0x8010001F  /* An unexpected card error has occurred. */
#define SCARD_E_UNKNOWN_CARD            0x8010000D  /* The specified smart card name is not recognized. */
#define SCARD_E_UNKNOWN_READER          0x80100009  /* The specified reader name is not recognized. */
#define SCARD_E_UNKNOWN_RES_MNG         0x8010002B  /* An unrecognized error code was returned. */
#define SCARD_E_UNSUPPORTED_FEATURE     0x80100022  /* This smart card does not support the requested feature. */
#define SCARD_E_WRITE_TOO_MANY          0x80100028  /* An attempt was made to write more data than would fit in the target object. */
#define SCARD_F_COMM_ERROR              0x80100013  /* An internal communications error has been detected. */
#define SCARD_F_INTERNAL_ERROR          0x80100001  /* An internal consistency check failed. */
#define SCARD_F_UNKNOWN_ERROR           0x80100014  /* An internal error has been detected, but the source is unknown. */
#define SCARD_F_WAITED_TOO_LONG         0x80100007  /* An internal consistency timer has expired. */
#define SCARD_P_SHUTDOWN                0x80100018  /* The operation has been aborted to allow the server application to exit. */   
#define SCARD_S_SUCCESS                 0           /* No error was encountered. */
#define SCARD_W_CANCELLED_BY_USER       0x8010006E  /* The action was canceled by the user. */
#define SCARD_W_CACHE_ITEM_NOT_FOUND    0x80100070  /* The requested item could not be found in the cache. */
#define SCARD_W_CACHE_ITEM_STALE        0x80100071  /* The requested cache item is too old and was deleted from the cache. */
#define SCARD_W_CACHE_ITEM_TOO_BIG      0x80100072  /* The new cache item exceeds the maximum per-item size defined for the cache. */
#define SCARD_W_CARD_NOT_AUTHENTICATED  0x8010006F  /* No PIN was presented to the smart card. */
#define SCARD_W_CHV_BLOCKED             0x8010006C  /* The card cannot be accessed because the maximum number of PIN entry attempts has been reached. */
#define SCARD_W_EOF                     0x8010006D  /* The end of the smart card file has been reached. */
#define SCARD_W_REMOVED_CARD            0x80100069  /* The smart card has been removed, so further communication is not possible. */
#define SCARD_W_RESET_CARD              0x80100068  /* The smart card was reset. */
#define SCARD_W_SECURITY_VIOLATION      0x8010006A  /* Access was denied because of a security violation. */
#define SCARD_W_UNPOWERED_CARD          0x80100067  /* Power has been removed from the smart card, so that further communication is not possible. */
#define SCARD_W_UNRESPONSIVE_CARD       0x80100066  /* The smart card is not responding to a reset. */
#define SCARD_W_UNSUPPORTED_CARD        0x80100065  /* The reader cannot communicate with the card, due to ATR string configuration conflicts. */
#define SCARD_W_WRONG_CHV               0x8010006B  /* The card cannot be accessed because the wrong PIN was presented. */

#pragma mark PCSC Constants
#define SCARD_ATR_LENGTH                36          /* Maximum ATR size */

#pragma mark PCSC Types
typedef int32_t SCARDHANDLE,  *PSCARDHANDLE, *LPSCARDHANDLE;
typedef int32_t SCARDCONTEXT, *PSCARDCONTEXT, *LPSCARDCONTEXT;

#pragma mark PCSC Structures
#pragma pack(1)

/* Protocol Control Information (PCI) */
typedef struct _SCARD_IO_REQUEST
{
    uint32_t dwProtocol;	/* Protocol identifier */
    uint32_t cbPciLength;	/* Protocol Control Inf Length */
}
SCARD_IO_REQUEST, *PSCARD_IO_REQUEST, *LPSCARD_IO_REQUEST;

typedef const SCARD_IO_REQUEST *LPCSCARD_IO_REQUEST;

extern SCARD_IO_REQUEST g_pksdk_rgSCardT0Pci, g_pksdk_rgSCardT1Pci, g_pksdk_rgSCardRawPci;

typedef struct {
    LPCTSTR szReader;
    LPVOID  pvUserData;
    DWORD   dwCurrentState;
    DWORD   dwEventState;
    DWORD   cbAtr;
    BYTE    rgbAtr[SCARD_ATR_LENGTH];
} SCARD_READERSTATE, *PSCARD_READERSTATE, *LPSCARD_READERSTATE;
    
#define SCARD_READER_NOTIFICATION_NAME "\\\\?PnP?\\Notification"

typedef struct _SCARD_ATRMASK {
    DWORD cbAtr;
    BYTE  rgbAtr[36];
    BYTE  rgbMask[36];
} SCARD_ATRMASK, *PSCARD_ATRMASK, *LPSCARD_ATRMASK;

#define SCARD_PCI_T0	(&g_pksdk_rgSCardT0Pci) /* protocol control information (PCI) for T=0 */
#define SCARD_PCI_T1	(&g_pksdk_rgSCardT1Pci) /* protocol control information (PCI) for T=1 */
#define SCARD_PCI_RAW	(&g_pksdk_rgSCardRawPci) /* protocol control information (PCI) for RAW protocol */

#pragma pack()

#pragma mark PCSC Constants

#define SCARD_SCOPE_USER		0x0000	/* Scope in user space */
#define SCARD_SCOPE_TERMINAL		0x0001	/* Scope in terminal */
#define SCARD_SCOPE_SYSTEM		0x0002	/* Scope in system */

#define SCARD_PROTOCOL_UNDEFINED	0x0000	/* protocol not set */
#define SCARD_PROTOCOL_UNSET SCARD_PROTOCOL_UNDEFINED	/* backward compat */
#define SCARD_PROTOCOL_T0		0x0001	/* T=0 active protocol. */
#define SCARD_PROTOCOL_T1		0x0002	/* T=1 active protocol. */
#define SCARD_PROTOCOL_RAW		0x0004	/* Raw active protocol. */
#define SCARD_PROTOCOL_T15		0x0008	/* T=15 protocol. */
#define SCARD_PROTOCOL_ANY		(SCARD_PROTOCOL_T0|SCARD_PROTOCOL_T1)	/* IFD determines prot. */

#define SCARD_SHARE_EXCLUSIVE		0x0001	/* Exclusive mode only */
#define SCARD_SHARE_SHARED		0x0002	/* Shared mode only */
#define SCARD_SHARE_DIRECT		0x0003	/* Raw mode only */

#define SCARD_LEAVE_CARD		0x0000	/* Do nothing on close */
#define SCARD_RESET_CARD		0x0001	/* Reset on close */
#define SCARD_UNPOWER_CARD		0x0002	/* Power down on close */
#define SCARD_EJECT_CARD		0x0003	/* Eject on close */

#define SCARD_UNKNOWN			0x0001	/* Unknown state */
#define SCARD_ABSENT			0x0002	/* Card is absent */
#define SCARD_PRESENT			0x0004	/* Card is present */
#define SCARD_SWALLOWED			0x0008	/* Card not powered */
#define SCARD_POWERED			0x0010	/* Card is powered */
#define SCARD_NEGOTIABLE		0x0020	/* Ready for PTS */
#define SCARD_SPECIFIC			0x0040	/* PTS has been set */

#define SCARD_STATE_UNAWARE		0x0000	/* App wants status */
#define SCARD_STATE_IGNORE		0x0001	/* Ignore this reader */
#define SCARD_STATE_CHANGED		0x0002	/* State has changed */
#define SCARD_STATE_UNKNOWN		0x0004	/* Reader unknown */
#define SCARD_STATE_UNAVAILABLE	0x0008	/* Status unavailable */
#define SCARD_STATE_EMPTY		0x0010	/* Card removed */
#define SCARD_STATE_PRESENT		0x0020	/* Card inserted */
#define SCARD_STATE_ATRMATCH	0x0040	/* ATR matches card */
#define SCARD_STATE_EXCLUSIVE	0x0080	/* Exclusive Mode */
#define SCARD_STATE_INUSE		0x0100	/* Shared Mode */
#define SCARD_STATE_MUTE		0x0200	/* Unresponsive card */
#define SCARD_STATE_UNPOWERED	0x0400	/* Unpowered card */

/* PC/SC Lite specific extensions */
#define SCARD_W_INSERTED_CARD		(long)0x8010007F

#define SCARD_SCOPE_GLOBAL		0x0003	/* Scope is global */

#define SCARD_RESET             0x0001	/* Card was reset */
#define SCARD_INSERTED			0x0002	/* Card was inserted */
#define SCARD_REMOVED			0x0004	/* Card was removed */
    
/* from Precise Biometrics */
    /** Tells the library to allocate the required memory */
#define SCARD_AUTOALLOCATE                  (DWORD)(-1)


#pragma mark PCSC Functions
/*
 * functions
 */

HANDLE WINAPI SCardAccessStartedEvent(void);

LONG WINAPI SCardAddReaderToGroup(
                                  _param_in  SCARDCONTEXT hContext,
                                  _param_in  LPCTSTR szReaderName,
                                  _param_in  LPCTSTR szGroupName
                                  );

LONG WINAPI SCardBeginTransaction(
                                  _param_in  SCARDHANDLE hCard
                                  );
LONG WINAPI SCardCancel(
                        _param_in  SCARDCONTEXT hContext
                        );

LONG WINAPI SCardConnect(
                         _param_in   SCARDCONTEXT hContext,
                         _param_in   LPCTSTR szReader,
                         _param_in   DWORD dwShareMode,
                         _param_in   DWORD dwPreferredProtocols,
                         _param_out  LPSCARDHANDLE phCard,
                         _param_out  LPDWORD pdwActiveProtocol
                         );

LONG WINAPI SCardControl(
                         _param_in   SCARDHANDLE hCard,
                         _param_in   DWORD dwControlCode,
                         _param_in   LPCVOID lpInBuffer,
                         _param_in   DWORD nInBufferSize,
                         _param_out  LPVOID lpOutBuffer,
                         _param_in   DWORD nOutBufferSize,
                         _param_out  LPDWORD lpBytesReturned
                         );

LONG WINAPI SCardDisconnect(
                            _param_in  SCARDHANDLE hCard,
                            _param_in  DWORD dwDisposition
                            );

LONG WINAPI SCardEndTransaction(
                                _param_in  SCARDHANDLE hCard,
                                _param_in  DWORD dwDisposition
                                );

LONG WINAPI SCardEstablishContext(
                                  _param_in   DWORD dwScope,
                                  _param_in   LPCVOID pvReserved1,
                                  _param_in   LPCVOID pvReserved2,
                                  _param_out  LPSCARDCONTEXT phContext
                                  );

LONG WINAPI SCardForgetCardType(
                                _param_in  SCARDCONTEXT hContext,
                                _param_in  LPCTSTR szCardName
                                );

LONG WINAPI SCardForgetReader(
                              _param_in  SCARDCONTEXT hContext,
                              _param_in  LPCTSTR szReaderName
                              );

LONG WINAPI SCardForgetReaderGroup(
                                   _param_in  SCARDCONTEXT hContext,
                                   _param_in  LPCTSTR szGroupName
                                   );

LONG WINAPI SCardFreeMemory(
                            _param_in  SCARDCONTEXT hContext,
                            _param_in  LPCVOID pvMem
                            );

LONG WINAPI SCardGetAttrib(
                           _param_in     SCARDHANDLE hCard,
                           _param_in     DWORD dwAttrId,
                           _param_out    LPBYTE pbAttr,
                           _param_inout  LPDWORD pcbAttrLen
                           );

LONG WINAPI SCardGetCardTypeProviderName(
                                         _param_in     SCARDCONTEXT hContext,
                                         _param_in     LPCTSTR szCardName,
                                         _param_in     DWORD dwProviderId,
                                         _param_out    LPTSTR szProvider,
                                         _param_inout  LPDWORD *pcchProvider
                                         );

LONG WINAPI SCardGetProviderId(
                               _param_in   SCARDCONTEXT hContext,
                               _param_in   LPCTSTR szCard,
                               _param_out  LPGUID pguidProviderId
                               );

LONG WINAPI SCardGetStatusChange(
                                 _param_in     SCARDCONTEXT hContext,
                                 _param_in     DWORD dwTimeout,
                                 _param_inout  LPSCARD_READERSTATE rgReaderStates,
                                 _param_in     DWORD cReaders
                                 );

LONG WINAPI SCardGetTransmitCount(
                                  _param_in   SCARDHANDLE hCard,
                                  _param_out  LPDWORD pcTransmitCount
                                  );

LONG WINAPI SCardIntroduceCardType(
                                   _param_in      SCARDCONTEXT hContext,
                                   _param_in      LPCTSTR szCardName,
                                   _param_in_opt  LPCGUID pguidPrimaryProvider,
                                   _param_in_opt  LPCGUID rgguidInterfaces,
                                   _param_in      DWORD dwInterfaceCount,
                                   _param_in      LPCBYTE pbAtr,
                                   _param_in      LPCBYTE pbAtrMask,
                                   _param_in      DWORD cbAtrLen
                                   );

LONG WINAPI SCardIntroduceReader(
                                 _param_in  SCARDCONTEXT hContext,
                                 _param_in  LPCTSTR szReaderName,
                                 _param_in  LPCTSTR szDeviceName
                                 );

LONG WINAPI SCardIntroduceReaderGroup(
                                      _param_in  SCARDCONTEXT hContext,
                                      _param_in  LPCTSTR szGroupName
                                      );

LONG WINAPI SCardIsValidContext(
                                _param_in  SCARDCONTEXT hContext
                                );

LONG WINAPI SCardListCards(
                           _param_in      SCARDCONTEXT hContext,
                           _param_in_opt  LPCBYTE pbAtr,
                           _param_in      LPCGUID rgguidInterfaces,
                           _param_in      DWORD cguidInterfaceCount,
                           _param_out     LPTSTR mszCards,
                           _param_inout   LPDWORD pcchCards
                           );

LONG WINAPI SCardListInterfaces(
                                _param_in     SCARDCONTEXT hContext,
                                _param_in     LPCTSTR szCard,
                                _param_out    LPGUID pguidInterfaces,
                                _param_inout  LPDWORD pcguidInterfaces
                                );

LONG WINAPI SCardListReaderGroups(
                                  _param_in     SCARDCONTEXT hContext,
                                  _param_out    LPTSTR mszGroups,
                                  _param_inout  LPDWORD pcchGroups
                                  );

LONG WINAPI SCardListReaders(
                             _param_in      SCARDCONTEXT hContext,
                             _param_in_opt  LPCTSTR mszGroups,
                             _param_out     LPTSTR mszReaders,
                             _param_inout   LPDWORD pcchReaders
                             );

LONG WINAPI SCardListReadersWithDeviceInstanceId(
                                                 _param_In_       SCARDCONTEXT hContext,
                                                 _param_In_       LPCTSTR szDeviceInstanceId,
                                                 _param_Out_opt_  LPTSTR mszReaders,
                                                 _param_Inout_    LPDWORD pcchReaders
                                                 );

LONG WINAPI SCardLocateCards(
                             _param_in     SCARDCONTEXT hContext,
                             _param_in     LPCTSTR mszCards,
                             _param_inout  LPSCARD_READERSTATE rgReaderStates,
                             _param_in     DWORD cReaders
                             );

LONG WINAPI SCardLocateCardsByATR(
                                  _param_in     SCARDCONTEXT hContext,
                                  _param_in     LPSCARD_ATRMASK rgAtrMasks,
                                  _param_in     DWORD cAtrs,
                                  _param_inout  LPSCARD_READERSTATE rgReaderStates,
                                  _param_in     DWORD cReaders
                                  );

LONG WINAPI SCardReadCache(
                           _param_in   SCARDCONTEXT hContext,
                           _param_in   UUID *CardIdentifier,
                           _param_in   DWORD FreshnessCounter,
                           _param_in   LPTSTR LookupName,
                           _param_out  PBYTE Data,
                           _param_out  DWORD *DataLen
                           );

LONG WINAPI SCardReconnect(
                           _param_in       SCARDHANDLE hCard,
                           _param_in       DWORD dwShareMode,
                           _param_in       DWORD dwPreferredProtocols,
                           _param_in       DWORD dwInitialization,
                           _param_out_opt  LPDWORD pdwActiveProtocol
                           );

LONG WINAPI SCardReleaseContext(
                                _param_in  SCARDCONTEXT hContext
                                );

void WINAPI SCardReleaseStartedEvent(void);

LONG WINAPI SCardRemoveReaderFromGroup(
                                       _param_in  SCARDCONTEXT hContext,
                                       _param_in  LPCTSTR szReaderName,
                                       _param_in  LPCTSTR szGroupName
                                       );

LONG WINAPI SCardSetAttrib(
                           _param_in  SCARDHANDLE hCard,
                           _param_in  DWORD dwAttrId,
                           _param_in  LPCBYTE pbAttr,
                           _param_in  DWORD cbAttrLen
                           );

LONG WINAPI SCardSetCardTypeProviderName(
                                         _param_in  SCARDCONTEXT hContext,
                                         _param_in  LPCTSTR szCardName,
                                         _param_in  DWORD dwProviderId,
                                         _param_in  LPCTSTR szProvider
                                         );

LONG WINAPI SCardStatus(
                        _param_in         SCARDHANDLE hCard,
                        _param_out        LPTSTR szReaderName,
                        _param_inout_opt  LPDWORD pcchReaderLen,
                        _param_out_opt    LPDWORD pdwState,
                        _param_out_opt    LPDWORD pdwProtocol,
                        _param_out        LPBYTE pbAtr,
                        _param_inout_opt  LPDWORD pcbAtrLen
                        );

LONG WINAPI SCardTransmit(
                          _param_in         SCARDHANDLE hCard,
                          _param_in         LPCSCARD_IO_REQUEST pioSendPci,
                          _param_in         LPCBYTE pbSendBuffer,
                          _param_in         DWORD cbSendLength,
                          _param_inout_opt  LPSCARD_IO_REQUEST pioRecvPci,
                          _param_out        LPBYTE pbRecvBuffer,
                          _param_inout      LPDWORD pcbRecvLength
                          );

#if UIDlgSelectCard_Supported
LONG WINAPI SCardUIDlgSelectCard(
                                 _param_in  LPOPENCARDNAME_EX pDlgStruc
                                 );
#endif

LONG WINAPI SCardWriteCache(
                            _param_in  SCARDCONTEXT hContext,
                            _param_in  UUID *CardIdentifier,
                            _param_in  DWORD FreshnessCounter,
                            _param_in  LPTSTR LookupName,
                            _param_in  PBYTE Data,
                            _param_in  DWORD DataLen
                            );


    
#ifdef __cplusplus
}
#endif

#endif
